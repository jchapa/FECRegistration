...
function setRegTypeOnChange() {
    ...
    case 0:
        ...
        break;
    case 1:
        $(".regular-price").text('$259');
        //$(".early-pricing").text('$299');
        $("#registration-total").text('$259');
        ...
        break;
    case 2:
        $(".regular-price").text('$99');
        //$(".early-pricing").text('$109');
        $("#registration-total").text('$99');
...
function setNumOfAttOnChange() {
    var num = document.getElementById("number-of-attendees").value;
    // Set Variables
    if (num) {
    ...
    // Compare LI Count and drop-down number to determine whether to append LI
    // or remove
...
$(document).ready(
                function() {
                    if (document.getElementById("wizardry").value != "bam") {
                        ...
                        var regType = document
                                .getElementById("registration-type").value;
                        if (regType === "family") {
                            if (attNum) {
                                $("select#registration-type").change(
                                        setRegTypeOnChange);
                                $("select#number-of-attendees").change(
                                        setNumOfAttOnChange(attNum));
                            } else {
                                $("select#registration-type").change(
                                        setRegTypeOnChange);
                                $("select#number-of-attendees").change(
                                        setNumOfAttOnChange(1));
                            }
                        } else {
                            setRegTypeOnChange();
                        }
                    ...
                    $.validator.addMethod(
                                    ...
                                    $.validator.methods.minlength,
                                    $
                                            .format("Last Name must have at least {0} characters"));
                    $.validator.addClassRules("person-name-first", {
                        fNameRequired : true,
                        fNameMinlength : 2
                    });
                    $.validator.addClassRules("person-name-last", {
                        lNameRequired : true,
                        lNameMinlength : 2
                    });
                                            ...
                                            "number-of-attendees" : {
                                                required : {
                                                    depends : function(element) {
                                                        return $("#registration-type")[0].selectedIndex != 2
                                                                && $("#registration-type")[0].selectedIndex != 0
                                                    }
                                                }
                                            },
                                            "all-days" : {
